...
func meanSpeed(action int, duration float64) float64 {
	...
	distanceLength := distance(action)
	return distanceLength / duration
}
...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Бег":
		distanceLength := distance(action)                         // вызовите здесь необходимую функцию
		...
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distanceLength, speed, calories)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedfirst := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedfirst, 2)/(height/mInKm))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
